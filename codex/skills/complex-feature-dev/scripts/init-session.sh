...
Usage:
  init-session.sh [--here] [--dir PATH] [--no-agents] [--force]

Options:
  --here        Create files in the current directory (skip git root detection)
  --dir PATH    Create files in a specific directory
  --agents      (legacy) Also create AGENTS.md if missing
  --no-agents   Skip creating AGENTS.md
...
# Optional/Default: create agent instruction files
